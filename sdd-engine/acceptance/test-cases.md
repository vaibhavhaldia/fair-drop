...
**Scale target.** Expected turnout is 50–250 humans, so **250–1,250 participants** at the fixed
4:1 bot ratio. Cases that specify a population use the top of that range unless the case is
...
| TC-SCH-07 | UT    | P1  | `event.currentSlotEndsAt` is optional and starts `null`; `event.totalTickets`, `event.ticketsRemaining`, `event.participantsAtOpen`, `slot.quota`, `slot.filled`, and `slot.entriesReceived` all start `0`.              |
...
| TC-LC-02 | UT    | P0  | `start_countdown` moves `created → countdown`, snapshots `participantsAtOpen`, sets `totalTickets = round(ticketFraction × participants)`, seeds per-slot quotas, and schedules `open_event` at `now + countdownSeconds` (60s default). |
...
| TC-LC-11 | UT    | P0  | `start_countdown` on an event already in `countdown` or `open` is rejected — inventory is sized exactly once and never resized mid-event.                                                              |
...
> hits 0 mid-schedule. With quotas derived from inventory (`sum(quota) == totalTickets`) and
...
| TC-EVT-02 | UT    | P0  | Turn-mode create with 5 floors persists 5 `slot` rows with the given floors, `quota = 0`, `filled = 0`, `entriesReceived = 0`, and `currentSlotIndex = 0`. Quotas stay 0 until `start_countdown`. |
...
| TC-EVT-09 | UT    | P0  | Quotas cannot exceed inventory **by construction**: after `start_countdown`, `sum(slot.quota) == event.totalTickets` exactly. There is no overbooking path to reject because quotas are derived from the total, not supplied alongside it. |
| TC-EVT-10 | UT    | P0  | `start_countdown` sizes inventory from live headcount: at 250 / 500 / 750 / 1250 participants with fraction 0.40 → `totalTickets` of 100 / 200 / 300 / 500, and per-slot quotas of 20 / 40 / 60 / 100. |
| TC-EVT-11 | UT    | P1  | Integer-division remainder is assigned to the **final** slot: 103 tickets over 5 slots → quotas `20, 20, 20, 20, 23`, summing to 103.                             |
| TC-EVT-12 | UT    | P1  | `start_countdown` with zero registered participants is rejected (or yields `totalTickets == 0` and settles immediately — specify which; it must not produce a live event with no inventory). |
...
| ID         | Level | P   | Case                                                                                                                                                                                                   |
| ---------- | ----- | --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| TC-JOIN-01 | UT    | P0  | `join("Asha", "human")` creates a participant with `origin = "human"`, `hasWon = false`, and `walletBalance` drawn from `randomUniform(20000, 150000)` — **not** a fixed 500000.                       |
| TC-JOIN-02 | UT    | P0  | `join("Bot-x9", "bot")` draws `walletBalance` from the **same** distribution — bots are neither privileged nor handicapped by balance.                                                                  |
| TC-JOIN-03 | UT    | P0  | Over 500 joins, `walletBalance` is approximately uniform on [20000, 150000]: the minimum draw never falls below Slot 1's floor (15000), and ~73% of draws clear the top floor (55000), so the last slot retains a real field (LLD §1a). |
| TC-JOIN-04 | UT    | P0  | The same identity calling `join` twice does not create a second participant and does not re-credit or re-draw the wallet (idempotent or rejected — either is acceptable, silent re-credit is not).      |
...
| TC-JOIN-09 | IT    | P1  | `submit_bid` from an identity that never joined is rejected ("registered participant" guard).                                                                                                          |
...
| TC-BID-02 | UT    | P0  | **C2** — the same identity submitting a second entry in the same slot is rejected; exactly one `bid` row persists, and it is the first one (the second is not queued and does not overwrite). |
...
| TC-CLR-10 | UT    | P0  | **Seed independence** — `drawSeed` derives only from `(eventId, slotIndex, sorted(entry ids))`. Assert by construction that `seq`, insertion order, participant identity, and join order are not inputs, and that the allocator never dereferences them for ordering.                     |
...
| TC-ROLL-07 | IT    | P1  | Rollover survives the derived-quota path: after `start_countdown` sizes quotas, an under-filled slot still rolls correctly at any turnout (250 / 1250 participants). |
...
| TC-WAL-03 | UT    | P0  | Debit happens **exactly once** per allocation: `walletBalance == initialDraw - sum(pricePaid of that participant's allocations)` at all times. Note `initialDraw` is the per-participant random draw, not a shared constant — the assertion must read the participant's own join-time value. |
...
| TC-INV-04 | IT    | P0  | **C1, system-level (Merge Gate 5).** Bots at near-zero reaction delay vs. humans entering late in the window, same slot → win rates are indistinguishable. A bot entering at t+50ms has exactly the same odds as a human entering at t+58s.                                                                                                 |
...
| TC-INV-06 | UT    | P0  | **C2.** One entry per identity per slot — see TC-BID-02; asserted here again as an invariant sweep: `count(bid) grouped by (eventId, slotIndex, participant) <= 1` after every test.                                                                                                                                                        |
...
| TC-INV-10 | UT    | P0  | **C5.** `allocations(identity, event) <= 1` swept after every test, in both modes. A winner is rejected by `submit_bid` in all later slots and skipped by `close_slot` if an entry somehow persists.                                                                                                                                        |
...
| ID         | Level | P   | Case                                                                                                                                                                                                                            |
| ---------- | ----- | --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TC-POOL-01 | IT    | P0  | `POST /onboard { displayName }` returns `{ identity, walletBalance }` for the human and creates exactly one `origin: "human"` participant.                                                                                      |
...
| C2 — one entry per identity per slot  | TC-BID-02              | TC-INV-06, TC-UI-10, TC-BOT-06                                  |
...
| C5 — one ticket per identity per event| TC-INV-10              | TC-BID-10, TC-Q-07, TC-CLR-12, TC-UI-13, TC-WAL-07              |
